...
export default function Home() {
  ...
  const getAllBooks = async () => {
    setIsLoading(true);
    try {
      const { data } = await bookBaseUrl.get('/booklists'); // GET /api/book/booklists
      setBookList(data.BookList || []);
    } catch (err) {
      toast.error(err.response?.data?.Message || 'Failed to fetch books');
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    const endpoint = isUpdating ? '/updatebook' : '/addbook'; // no extra /book
    try {
      const { data } = isUpdating
        ? await bookBaseUrl.put(endpoint, bookForm) // PUT /api/book/updatebook
        : await bookBaseUrl.post(endpoint, bookForm); // POST /api/book/addbook
  ...
  const handleDelete = async id => {
    ...
    setIsLoading(true);
    try {
      const { data } = await bookBaseUrl.post('/deletebook', { Id: id }); // POST /api/book/deletebook
      if (data.Success) {
        toast.info(data.Message);
        getAllBooks();
      } else {
