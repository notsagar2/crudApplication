...
async function handleBookStoreController(req, res) {
  try {
    ...
    if (!BookName || !BookTitle || !SellingPrice) {
      return res
        .status(400)
        .json({ Message: 'Name, Title & Price are required', Success: false });
    }
    ...
    return res
      .status(201)
      .json({ Message: 'Created successfully', Success: true, Book: newBook });
  } catch (error) {
    return res
      .status(500)
      .json({ Message: error.message || 'Internal error', Success: false });
  }
}
...
async function handleBookDeleteController(req, res) {
  try {
    ...
    if (!Id)
      return res
        .status(400)
        .json({ Message: 'Id required', Success: false });
...
async function handleBookUpdateController(req, res) {
  try {
    ...
    const updated = await Book.findByIdAndUpdate(Id, updates, {
      new: true,
      runValidators: true,
    });
    if (!updated)
